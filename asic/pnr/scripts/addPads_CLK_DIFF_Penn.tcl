...
catch { selectInstByCellName  CLK_DIFF_Penn}
...
foreach ioInst [dbGet selected] {
    ...
    if {$orient == "R180"} {
        ## First pad at x+25
        addInst -physical -cell PAD50LU -inst pad_${name}_a \
            -loc [expr {$x + 45}] [expr {$y + 121.5}] -ori $orient -status fixed

        ## Second pad at x+125
        addInst -physical -cell PAD50LU -inst pad_${name}_b \
            -loc [expr {$x + 145}] [expr {$y + 121.5}] -ori $orient -status fixed
    } 
    ...
    if {$orient == "R0"}   {
        ## First pad at x+25
        addInst -physical -cell PAD50LU -inst pad_${name}_a \
            -loc [expr {$x + 25}] $y -ori $orient -status fixed

        ## Second pad at x+125
        addInst -physical -cell PAD50LU -inst pad_${name}_b \
            -loc [expr {$x + 125}] $y -ori $orient -status fixed
    } 
